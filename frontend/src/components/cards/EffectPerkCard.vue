...
function apply() {
  ...
  save.act(() => effperk.apply(), t('log.effperkApplied', { n }))
}

onMounted(effperk.load)
save.onReload(effperk.load)
</script>

<template>
  <CardShell id="effectPerkCard" :title="t('effperk.cardTitle')" :dirty="effperk.dirty">
    <template #action>
      <NButton size="small" type="primary" secondary :disabled="!effperk.dirty" @click="apply">
        {{ t('common.apply') }}
      </NButton>
    </template>
    <EffectPerkWall />
